...
function normalizeSection(value) {
    ...
    const number = +text;
    return number >= 1 && number <= XCU_TIME_SLOTS.length ? number : NaN;
}
...
// 按用户提供的许昌学院课表截图逐节录入：白天按截图并经用户确认，晚间19:00开始，每节45分钟，间隔5分钟。
const XCU_TIME_SLOTS = [
    { number: 1, startTime: "08:00", endTime: "08:50" },
    { number: 2, startTime: "09:00", endTime: "09:30" },
    { number: 3, startTime: "09:50", endTime: "10:20" },
    { number: 4, startTime: "11:00", endTime: "11:20" },
    { number: 5, startTime: "11:30", endTime: "12:10" },
    { number: 6, startTime: "14:30", endTime: "15:20" },
    { number: 7, startTime: "15:40", endTime: "16:00" },
    { number: 8, startTime: "16:20", endTime: "16:50" },
    { number: 9, startTime: "17:20", endTime: "17:50" },
...
async function runImportFlow() {
    const alertConfirmed = await window.shiguangBridgePromise.showAlert(
        "许昌学院课表导入（测试版）",
        "请先登录许昌学院教务，打开个人课表并选择学期、点击查询。将导入课程和已确认的第1至12节作息。开学日期请在软件内核对设置。",
        "好的，开始导入"
    );
    if (!alertConfirmed) {
    ...
    if (invalidCourses.length > 0) {
        ...
        await window.shiguangBridgePromise.showAlert("节次超出已确认作息",
            "本校已配置第1至12节。以下课程节次异常，本次未保存，请截图联系维护者核对：\n" + details, "确定");
        return;
    }
    ...
    try {
        await window.shiguangBridgePromise.savePresetTimeSlots(JSON.stringify(XCU_TIME_SLOTS));
    } catch (error) {
        await window.shiguangBridgePromise.showAlert("作息保存失败",
            "课程已保存，但固定作息未保存成功，请重试导入并核对时间。错误：" + error.message, "确定");
        return;
    }
    window.shiguangBridge.showToast(`课程和已确认作息导入成功，共导入 ${courses.length} 门课程！`);
